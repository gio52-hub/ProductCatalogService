...
func (r *ProductRepo) FindByID(ctx context.Context, id string) (*domain.Product, error) {
	row, err := r.client.Single().ReadRow(
		ctx,
		models.ProductsTable,
		spanner.Key{id},
		models.ProductAllColumns(),
	)
	if err != nil {
...
func (r *ProductRepo) UpdateMut(product *domain.Product) *spanner.Mutation {
	...
	if changes.Dirty(domain.FieldName) {
		updates[models.ProductName] = product.Name()
	}

	if changes.Dirty(domain.FieldDescription) {
		updates[models.ProductDescription] = product.Description()
	}

	if changes.Dirty(domain.FieldCategory) {
		updates[models.ProductCategory] = product.Category()
	}

	if changes.Dirty(domain.FieldBasePrice) {
		updates[models.ProductBasePriceNum] = product.BasePrice().Numerator()
		updates[models.ProductBasePriceDenom] = product.BasePrice().Denominator()
	}
	...
	if changes.Dirty(domain.FieldDiscount) {
		...
		if discount != nil {
			...
			updates[models.ProductDiscountPercent] = spanner.NullNumeric{
				Numeric: *big.NewRat(int64(pct*100), 100),
				Valid:   true,
			}
			updates[models.ProductDiscountStartDate] = spanner.NullTime{Time: discount.StartDate(), Valid: true}
			updates[models.ProductDiscountEndDate] = spanner.NullTime{Time: discount.EndDate(), Valid: true}
		} else {
			updates[models.ProductDiscountPercent] = spanner.NullNumeric{Valid: false}
			updates[models.ProductDiscountStartDate] = spanner.NullTime{Valid: false}
			updates[models.ProductDiscountEndDate] = spanner.NullTime{Valid: false}
		}
	}

	if changes.Dirty(domain.FieldStatus) {
		updates[models.ProductStatus] = product.Status().String()
		if product.IsArchived() && product.ArchivedAt() != nil {
			updates[models.ProductArchivedAt] = spanner.NullTime{Time: *product.ArchivedAt(), Valid: true}
		}
	}
	...
	updates[models.ProductUpdatedAt] = product.UpdatedAt()
...
func (r *ProductRepo) ArchiveMut(product *domain.Product) *spanner.Mutation {
	updates := map[string]interface{}{
		models.ProductStatus:    product.Status().String(),
		models.ProductUpdatedAt: product.UpdatedAt(),
	}
...
func (r *ProductRepo) productToData(product *domain.Product) *models.ProductData {
...
func (r *ProductRepo) dataToDomain(data *models.ProductData) (*domain.Product, error) {
