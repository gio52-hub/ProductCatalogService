...
// ProductReadModel implements the contracts.ProductReadModel interface using Spanner.
...
func (rm *ProductReadModel) GetProduct(ctx context.Context, id string, at time.Time) (*contracts.ProductDTO, error) {
...
func (rm *ProductReadModel) ListProducts(ctx context.Context, filter contracts.ListProductsFilter, pagination contracts.Pagination, at time.Time) (*contracts.ListProductsResult, error) {
	stmt := rm.buildListQuery(filter, pagination)
	iter := rm.client.Single().Query(ctx, stmt)
	defer iter.Stop()

	products := make([]*contracts.ProductDTO, 0)
...
func (rm *ProductReadModel) ListByCategory(ctx context.Context, category string, pagination contracts.Pagination, at time.Time) (*contracts.ListProductsResult, error) {
...
func (rm *ProductReadModel) buildListQuery(filter contracts.ListProductsFilter, pagination contracts.Pagination) spanner.Statement {
...
func (rm *ProductReadModel) rowToDTO(row *spanner.Row, at time.Time) (*contracts.ProductDTO, error) {
	var (
		productID            string
		name                 string
		description          string
