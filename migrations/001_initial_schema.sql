...
-- Index for querying pending outbox events
CREATE INDEX idx_outbox_status ON outbox_events(status, created_at);

-- Index for querying products by category and status
CREATE INDEX idx_products_category ON products(category, status);

-- Index for querying active products
CREATE INDEX idx_products_status ON products(status) WHERE status != 'archived';
